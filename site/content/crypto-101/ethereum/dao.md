...
{{< img src="crypto-101/infographics/info-eth4-1.png" class="img-fluid my-3" alt="DAO" >}}
...
The organisation's purpose was to become the first decentralised venture capital, where participants could decide which projects would raise funds.  
...
3. At the end of the public crowdsale, a smart contract disbursed the tokens automatically to investors: the DAO went live.
4. Anyone could raise funds on the DAO platform: token holders could either accept or reject proposals.

The crowdsale was launched on the 30th of April 2016 and closed on the 28th of May, when the tokens were sent to contributors.  
It was such a success that the DAO gathered more than 18,000 investors and approximately 11.5 million Ether: that represented more than 14% of the total Ether supply issued to date.   
...
Unfortunately, this vulnerability got exploited only weeks after the DAO launch.  

An unknown attacker hacked the smart contract that held the funds and started draining Ether from the crowdsale.  
Nothing could be done, as altering smart contracts code is not that easy once it is deployed on a blockchain. Token holders witnessed the attack live, without being able to do much about it.  

Some attempted to separate the funds by pitching a project to the DAO, but this required token holders to vote and reach consensus: an action that was not feasible on such short notice.  
...
The attacker moved the funds to a copycat DAO, nicknamed “The Child DAO” – where all stolen Ether were locked during a 28-day crowdsale period (the vulnerability was to replicate the original DAO). 
...
<div class="container my-4">
  <div class="row align-items-center">
    <div class="col text-left">
      <h3>Hard fork</h3>
      <p>The second solution was more brutal: the idea was to rewind the blockchain back to a block before the DAO hack. The hard fork would therefore delete all transactions that occurred after the attack and return to DAO token holders the Ether they invested in the project. Similarly nodes would need to reach a consensus.</p>
    </div>
    <div class="col text-center">
...
Today date, there are now two Ethereum blockchains:
...
This is still a debatable subject, but as long as miners invest their time, energy and hardware on the network and people believe in the project, the Ethereum classic blockchain will have value.
